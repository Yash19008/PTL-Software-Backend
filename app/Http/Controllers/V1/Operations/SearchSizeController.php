...
class SearchSizeController extends Controller {
    ...
    public function searchSizeCommon($company, $userlength, $userwidth, $lower_range, $upper_range, $lower_range_reel, $upper_range_reel, $where, $gsm)
    {
        ...
        foreach ($admin_show_stocks_from as $from) {
            if ($company == '' || $company == $from) {
                switch ($from) {
                    case 'PTL':
                        $result = $this->get_data_from_connection('ptl_connection', 'searchSizeQuery', ['from' => $from, 'userlength' => $userlength, 'userwidth' => $userwidth, 'lower_range' => $lower_range, 'upper_range' => $upper_range, 'where' => $where]);
                        $output = array_merge($output, $result);
                        $result = $this->get_data_from_connection('ptl_connection', 'searchSizeQueryReel', ['from' => $from, 'userlength' => $userlength, 'userwidth' => $userwidth, 'lower_range_reel' => $lower_range_reel, 'upper_range_reel' => $upper_range_reel, 'where' => $where, 'gsm' => $gsm]);
    ...
    public function search_new_common(Request $request, $userlength, $userwidth, $searchReel, $return = 'api')
    {
        ...
        foreach($mobile_show_stocks_from as $from) {
            switch ($from) {
                case 'PTL':
                    $result = $this->get_data_from_connection('ptl_connection', 'search_new_common_query', ['from' => $from, 'userlength' => $userlength, 'userwidth' => $userwidth, 'lower_range' => $lower_range, 'upper_range' => $upper_range, 'where' => $where, 'sheets_result_count' => $sheets_result_count]);
                    $output = array_merge($output, $result);
                    if ($searchReel) {
                        $result = $this->get_data_from_connection('ptl_connection', 'reel_search_new_common_query', ['from' => $from, 'userlength' => $userlength, 'userwidth' => $userwidth, 'reel_lower_range' => $reel_lower_range, 'reel_upper_range' => $reel_upper_range, 'where' => $where, 'reels_result_count' => $reels_result_count, 'gsm' => $gsm]);
                        $reel_output = array_merge($reel_output, $result);
                    }
                break;
                case 'Paper Hub':
